package com.examly.springapp.Dhisharepository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
// import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

import com.examly.springapp.Dhishamodel.DhishaDoor;

// import jakarta.transaction.Transactiona

@Repository
public interface DoorRepo extends JpaRepository<DhishaDoor, Integer> {

    @Query(value = "DELETE FROM Door WHERE Door_id = ?1", nativeQuery = true)
    void deleteDoor(int DoorId);

    public List<DhishaDoor> findByColor(String color);

    public List<DhishaDoor> findByDoorType(String doortype);
}
